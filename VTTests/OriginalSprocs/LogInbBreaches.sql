...
/****** Object:  StoredProcedure [dbo].[vt_live_LogINBBreaches]    Script Date: 02/17/2017 17:08:22 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[vt_live_LogINBBreaches]
...
BEGIN
	...
	insert into #temp select distinct p.policy_ref, p.policy_account_handler_key,
					...
					and p.primary_introducer_key <> 6744671
					and p.policy_broker_key = 5920
	...
	WHILE EXISTS(SELECT TOP 1 * FROM #TEMP) 
		BEGIN 
			...
			INSERT INTO dbo.vt_live_breach_log (rule_code, context_ref, handler_key, office_key, team_key, broker_key, log_date) VALUES ('INB', @context, @handler, @office, @office, @broker, getdate())
			IF (select count(*) from dbo.vt_live_inb_archive where context_ref = @context) = 0
			BEGIN
				...
				insert into dbo.vt_live_inb_archive (context_ref, version_ref, client_key, client_name, product_target) 
											values (@context, @version, @client, dbo.GetClientName(@client), @product)
			END
			ELSE
			BEGIN
				update dbo.vt_live_breach_log set handler_key = @handler, office_key = @office where rule_code='INB' and context_ref=@context
				update dbo.vt_live_inb_archive set client_name=dbo.getclientname(@client), version_ref = @version, product_target = @product where context_ref=@context
			END
