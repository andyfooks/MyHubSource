...
/****** Object:  StoredProcedure [dbo].[vt_live_LogINNBreaches]    Script Date: 02/17/2017 17:08:46 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[vt_live_LogINNBreaches]
...
BEGIN
	...
	insert into #temp select distinct p.policy_ref, p.policy_account_handler_key,
					...
					and ((p.accepted_date_key <= DATEADD(dd,-7,GETDATE()) and p.accepted_date_key > DATEADD(dd, -14, GETDATE()))
						or (
						p.accepted_date_key <= DATEADD(dd, -14, Getdate()) and p.policy_ref in (select context_ref from VirtualTrainer.dbo.vt_live_breach_log where rule_code = 'INN')
					))
					and p.version_ref = p.last_in_band_version_ref
					and p.business_event_key <> 9 --no fees
					and p.primary_introducer_key = -1
					and p.client_key <> 2437461
					and p.client_key <> 2507575
					and p.client_key <> 2508197
					and p.client_key <> 2595141	--exceptions
					and p.policy_broker_key = 4198
	...
	WHILE EXISTS(SELECT TOP 1 * FROM #TEMP) 
		BEGIN 
			...
			INSERT INTO dbo.vt_live_breach_log (rule_code, context_ref, handler_key, office_key, team_key, broker_key, log_date) VALUES ('INN', @context, @handler, @office, @office, @broker, getdate())
			IF (select count(*) from dbo.vt_live_inn_archive where context_ref = @context) = 0
			BEGIN
				...
				insert into dbo.vt_live_inn_archive (context_ref, version_ref, client_key, client_name, product_target) 
											values (@context, @version, @client, dbo.GetClientName(@client), @product)
			END
			ELSE
			BEGIN
				update dbo.vt_live_breach_log set handler_key = @handler, office_key = @office where rule_code='INN' and context_ref=@context
				update dbo.vt_live_inn_archive set client_name=dbo.getclientname(@client), version_ref = @version, product_target = @product where context_ref=@context
			END
